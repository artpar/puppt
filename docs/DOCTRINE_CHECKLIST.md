...
| Tests cover the behavior at the right risk level | Inspection has focused tests for slide order, shape-level visible text, metadata, notes, image refs, layout refs/names, repeated text, unsupported-part warnings, and CLI JSON |
...
- Slide masters, full media classification, advanced non-text object extraction, and broader real-world unsupported-feature warning detection are not implemented yet.
