...
Set `PUPPT_FONT_MAP` to pin exact renderer font files for environments that provide Office-compatible fonts. Entries are semicolon-separated `family=/path/to/font.ttf` values; style-specific entries use `family:bold=...`, `family:italic=...`, or `family:bolditalic=...`. When exact Calibri-family fonts are unavailable, Puppt prefers locally installed Carlito metric-compatible fonts and then falls back to its bundled Carlito files before reporting an unresolved generic fallback.
